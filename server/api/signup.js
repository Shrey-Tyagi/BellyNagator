...
router.post("/signup",[
  ...
  check("email").isEmail({min: 6, max:60}).withMessage("Email must be a minimum length of 6 chars and a max of 60 chars."),
  ...
  db.connect(async (err, client, release) => {
    if (err) {
      ...
      return res.status(422).json({ errors: ["Unexpected error. Please try again."] });
    }
