...
// Part of code which query to usda taken help from https://stackoverflow.com/questions/37493970/using-usda-rest-api
...
router.post("/food/fooditem/:fdcId/mealtype/:mealType/date/:date", async (req, res) => {
  try {
    ...
    // Assign nutrients to corresponding formattedFood value. Taken from UDSA website and modified.
  ...
  formattedFood.foodPortions.push(["Default", 100.0]); 
...
router.post("/food/addfood/:fdcId/mealtype/:mealType/date/:date", [
  ...
  try {
    db.connect(async (err, client, release) => {
      ...
      release();
      return res.status(422).json({ msg: ["Voila food added"] });
    });
  } catch(e) {
      return res.status(422).json({ errors: ["Failed to add food. Please try again."] });
  }
})
